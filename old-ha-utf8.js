...
class HaVitodensProEditor extends HTMLElement {
  ...
  render() {
    ...
    const tabsHtml = `
      <div class="tabs">
        <button class="tab ${this.activeGroup === 'main' ? 'active' : ''}" data-group="main">Hlavn├Ż kotol</button>
    ...
    const allContentHtml = `
      <div class="tab-content ${this.activeGroup === 'main' ? 'active' : ''}" data-content="main">
        <div class="group"><h3>Hlavn├Ż kotol</h3>${generatePicker("Stav kotla (Entity)", "boiler_state")}</div>
      </div>
      <div class="tab-content ${this.activeGroup === 'sensors' ? 'active' : ''}" data-content="sensors">
        <div class="group"><h3>─îidl├í (Energia, teploty, spotreba)</h3>${SENSORS.map(s => generatePicker(s.name, s.id)).join('')}</div>
      </div>
      <div class="tab-content ${this.activeGroup === 'controls' ? 'active' : ''}" data-content="controls">
        <div class="group"><h3>Ovl├ídanie a nastavenia</h3>${CONTROLS.map(c => generatePicker(c.name, c.id)).join('')}</div>
      </div>
      ...
      <div class="tab-content ${this.activeGroup === 'rads' ? 'active' : ''}" data-content="rads">
        <div class="group"><h3>Radi├ítory samostatne (Max 5)</h3>${[1, 2, 3, 4, 5].map(i => generatePicker(`Teplota Radi├ítor ${i}`, `radiator_${i}_temp`)).join('')}</div>
      </div>
    `;
